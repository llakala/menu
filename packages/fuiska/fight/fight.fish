...
if [ -z "$ref" ]; or [ "$ref" = HEAD ]
    ...
    # `refs/branches/update/master`. To work around this, we only use the second
    # element in the list if it ends with `^{}`. If it doesn't, we use the first
    # element.
    #
    # This assumes ref_hashes will only ever be 1 or 2. If you reproduce it
    # being 3+ with an input of yours, make an issue!
    if [ (count $ref_hashes) = 2 ] && string match -q --regex "\^{}\$" $ref_names[-1]
